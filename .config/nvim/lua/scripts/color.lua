...
local function get_fg(r, g, b)
	...
	if package.loaded["scripts.highlights"] then
		...
		local _bg = highlights.srgb_to_oklab(highlights.get_property("bg", { "Normal" }, "#CDD6F4", "#1E1E2E"))

		local _fg = highlights.get_property("fg", { "Normal" }, "#1E1E2E", "#CDD6F4")

		if this.L >= _bg.L then
			return highlights.rgb_to_hex(highlights.oklab_to_srgb(_bg))
		else
...
color.color_line = function(buffer, lnum)
	...
	local text = vim.api.nvim_buf_get_lines(buffer, lnum, lnum + 1, false)[1] or ""
	...
	local function color_pattern_ranges(key)
		...
		while current_column < #text do
			...
			if not col_start or not col_end then
				return
			end
			...
			local found_hl, group = pcall(hl, buffer, match, style, lnum, ID)

			if found_hl then
				pcall(vim.api.nvim_set_hl, 0, group.name, group.value)
				pcall(render, buffer, group.name, style, lnum, col_start, col_end)
			end
