...
tree.stringify = function(node, opts)
	...
	if node:named() then
		table.insert(
			lines,
			string.format("(%s ; [ %d, %d ] - [ %d, %d ]", kind, range[1], range[2], range[3], range[4])
		)
	else
		table.insert(
			lines,
			string.format("%s ; [ %d, %d ] - [ %d, %d ]", '"' .. escape_str(kind) .. '"', range[1], range[2], range[3], range[4])
		)
	end
...
function instance:__named(node)
	...
	local node_config = vim.tbl_extend("force", default_config, match(self.config.named, node_name))
...
function instance:__anon(node)
	...
	local node_config = vim.tbl_extend("force", default_config, match(self.config.anonymous, node_name))
...
function instance:__injection(lnum, data)
	...
	local lang_config = vim.tbl_extend("force", default_config, match(self.config.injections, language))
...
function instance:highlight_node()
	...
	local cursor = vim.api.nvim_win_get_cursor(self.window)
	if not self.__data[cursor[1]] then
		return
	end
...
function instance:render()
	...
	local lines, data = tree.stringify(TSTree:root(), {
